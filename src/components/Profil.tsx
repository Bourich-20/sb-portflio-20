...
const Profil: React.FC<{ isDarkMode: boolean }> = ({ isDarkMode }) => {
  ...
  <motion.div
  initial={{ opacity: 0, scale: 0.8 }} // Ajout d'une échelle pour un effet d'apparition
  animate={{ opacity: 1, scale: 1 }} // Remise à l'échelle
  transition={{ duration: 1 }}
  ...
  max-h-[590px] overflow-hidden rounded-lg shadow-[0 10px 20px rgba(0,0,0,0.5)] border border-gray-200 transform transition-transform duration-300 hover:scale-105`} // Ajout de l'effet de survol
>
  <Image
    src={bourihImage}
    alt="Profile"
